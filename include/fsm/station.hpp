...
private:
    ...
    int rightBranchDelay = 0;       // 右分支框到达阈值后延迟（帧数）

    // Station停车可调参数：只改这里的数值，不改下面各分支停车逻辑
    struct Tune
    {
        // 通用
        static constexpr int STARTUP_IGNORE_FRAMES = 60;      // 发车后屏蔽station检测帧数
        static constexpr int NORMAL_TRIGGER_BOTTOM_MARGIN = 20; // 普通station距离底部触发余量
        ...
        static constexpr int BUSY_TARGET_BOTTOM_MARGIN = 130;  // 施工区目标框距离底部触发余量，增大可提前停车
        static constexpr int BUSY_FIRST_PRESS_FRAMES = 25;    // 施工区第一个框压框后停车等待帧数
        static constexpr int BUSY_TARGET_PRESS_FRAMES = 10;   // 施工区目标框压框后停车等待帧数
        static constexpr int BUSY_COOLDOWN_FRAMES = 6;        // 施工区停车后冷却帧数

        // 左岔路
        static constexpr float LEFT_BRANCH_TRIGGER_RATIO = 0.4f;
        static constexpr int LEFT_BRANCH_DELAY_FRAMES = 2;
        static constexpr int LEFT_BRANCH_PRESS_FRAMES = 36;

        // 右岔路
        static constexpr float RIGHT_BRANCH_TRIGGER_RATIO = 0.7f;
        static constexpr int RIGHT_BRANCH_DELAY_FRAMES = 15;
        static constexpr int RIGHT_BRANCH_PRESS_FRAMES = 10;
    };
