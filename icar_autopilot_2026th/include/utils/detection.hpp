...
public:
    ...
    Detection(const std::string pathModel, float score_nms)
    {
        score = score_nms;
        // 模型初始化
        this->predictor_nna_ = std::make_shared<PPNCPredictor>("../res/models/config_ppncnna.json");
        this->predictor_nms_ = std::make_shared<PPNCPredictor>("../res/models/config_ppncnms.json");
    ...
    void drawBox(Mat &img)
    {
        for (int i = 0; i < results.size(); i++)
        {
            ...
            if (result.y < 15)
                result.y = 15;
            cv::putText(img, label_name, Point(result.x, result.y), cv::FONT_HERSHEY_PLAIN, 1, cv::Scalar(0, 0, 0), 1);
        }
    }
    ...
    cv::Scalar getCvcolor(int index)
    {
        ...
        case 1:
            ...
            break;
        case 2:
            return cv::Scalar(0, 0, 255); // 汽车: 红色
            break;
        case 3:
            return cv::Scalar(0, 255, 255); // 施工区标志: 黄色
            break;
        case 4:
            return cv::Scalar(0, 255, 255); // 连续弯道: 黄色
            break;
        case 5:
